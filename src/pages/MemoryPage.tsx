  ...
  useEffect(() => {
    const handleSync = (event: MessageEvent) => {
      if (event.data.type === "AUTOSOLVE_CACHE_UPDATED") {
        ...
        if (
          keys.includes("autosolve_profile_cache") ||
          keys.includes("autosolve_alias_cache")
        ) {
          console.log(
            "🔄 [Wizard] Extension cache updated, refreshing view...",
          );
    ...
    try {
      ...
      for (const [key, value] of Object.entries(formFields)) {
        if (!configKeys.includes(key)) {
          ...
          if (stringValue) {
            learnedPayload = promoteLearnedValue(
              learnedPayload,
              key,
              stringValue,
            );

...
export const profileService = {
  ...
  async getProfileFromExtension(): Promise<ProfileDTO | null> {
    return new Promise((resolve) => {
      ...
      const handleMessage = (event: MessageEvent) => {
        if (event.data.type === "AUTOSOLVE_PROFILE_RESPONSE") {
      ...
      window.postMessage({ type: "AUTOSOLVE_GET_PROFILE" }, "*");
    });
  },
  ...
  async getPreferencesFromExtension(): Promise<PreferencesDTO | null> {
    return new Promise((resolve) => {
      ...
      const handleMessage = (event: MessageEvent) => {
        if (event.data.type === "AUTOSOLVE_PREFERENCES_RESPONSE") {
          clearTimeout(timeout);
          window.removeEventListener("message", handleMessage);
          resolve(event.data.payload);
        }
      ...
      window.addEventListener("message", handleMessage);
      window.postMessage({ type: "AUTOSOLVE_GET_PREFERENCES" }, "*");
    });
  },
  ...
  async updateProfile(data: UserProfileResponse): Promise<UserProfileResponse> {
    ...
    if (data.data_learned) {
      window.postMessage(
        { type: "AUTOSOLVE_SAVE_PROFILE", payload: data.data_learned },
        "*",
      );
    }
    ...
    if (data.preferences) {
      window.postMessage(
        { type: "AUTOSOLVE_SAVE_PREFERENCES", payload: data.preferences },
        "*",
      );
    }
